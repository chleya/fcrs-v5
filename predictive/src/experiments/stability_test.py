...
def run_experiment(selection_mode: str, seed: int, n_episodes: int = 200) -> Dict:
    """用指定种子运行实验"""
    np.random.seed(seed)
    ...
    return {
        'success_rate': agent.metrics.get_success_rate() * 100,
        'mean_reward': agent.metrics.get_mean_reward(),
        'mean_recon_error': agent.metrics.get_mean_recon_error()
    }


def main():
    print("="*60)
    print("FCRS-v5.3.1 Stability Test")
    print("="*60)
    ...
    for seed in seeds:
        print(f"\nSeed {seed}:")
        for mode in modes:
            result = run_experiment(mode, seed, n_episodes=200)
            all_results[mode].append(result['success_rate'])
            print(f"  {mode}: {result['success_rate']:.1f}%")
    ...
    for mode in modes:
        ...
        std = np.std(results)
        print(f"{mode:<20}: {mean:.1f}% +/- {std:.1f}%")
    
    # 核心对比
    pred_mean = np.mean(all_results['prediction'])
    recon_mean = np.mean(all_results['reconstruction'])
    
    print(f"\nPrediction vs Reconstruction: {pred_mean - recon_mean:+.1f}%")
