...
class OptimizedAgentFinal:
    def __init__(self, state_dim: int, action_dim: int,
                 selection_mode: str = 'prediction',
                 n_reps: int = 5, lr: float = 0.05, explore: float = 0.25):
        ...
        # 更强的策略网络初始化
        self.W_policy = np.random.randn(state_dim, action_dim) * 0.3
    ...
    def learn(self, state: np.ndarray, action: int, reward: float,
        ...
        for a in range(self.action_dim):
            if a == action:
                td_error = reward + gamma * np.max(q_values) - q_values[a]
                self.W_policy[:, a] += alpha * td_error * rep
                self.b_policy[a] += alpha * td_error
            else:
                self.W_policy[:, a] -= 0.001 * q_values[a] * rep
...
def run_experiment(selection_mode: str, n_episodes: int = 250, n_runs: int = 15) -> Dict:
...
def main():
    print("="*60)
    print("FCRS-v5.3.1 Final Optimization")
    print("="*60)
    ...
    for mode in modes:
        print(f"\n[{mode}] Running...")
        all_results[mode] = run_experiment(mode, n_episodes=250, n_runs=15)
        print(f"  Success: {all_results[mode]['success_rate']:.1f}% (+/- {all_results[mode]['std_success']:.1f}%)")
    ...
    for mode in modes:
        r = all_results[mode]
        print(f"{mode:<20} {r['success_rate']:.1f}% (+/-{r['std_success']:.1f}%)  Reward: {r['mean_reward']:.2f}")
