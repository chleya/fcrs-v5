...
def main():
    print("="*60)
    print("FCRS-v5.3.2 冲击60% - 更多episodes")
    print("="*60)
    
    # 测试更多episodes
    for ep in [300, 400, 500]:
        print(f"\n=== Episodes={ep} ===")
        pred = run_exp('prediction', n_reps=5, lr=0.05, explore=0.3, n_episodes=ep)
        print(f"Prediction: {pred:.1f}%")
    
    # 测试更低探索率
    for exp in [0.2, 0.15, 0.1]:
        print(f"\n=== Explore={exp} ===")
        pred = run_exp('prediction', n_reps=5, lr=0.05, explore=exp, n_episodes=200)
        print(f"Prediction: {pred:.1f}%")
